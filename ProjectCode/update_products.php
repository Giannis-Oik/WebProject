            ...
            if (move_uploaded_file($file_tmp, $upload_directory . $file_name)) {
                ...
                if ($array === null && json_last_error() !== JSON_ERROR_NONE) {
                    ...
                    foreach ($array as $row) {
                        ...
                        $check_sql = "SELECT * FROM products WHERE id = '$id' AND category = '$category' AND subcategory = '$subcategory'";
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            ...
                            if ($existingRow['name'] !== $name || $existingRow['id'] !== $id || $existingRow['category'] !== $category) {
                                // At least one field is different, perform an update
                                $update_sql = "UPDATE products SET name = '$name' WHERE id = '$id' AND category = '$category'  AND subcategory = '$subcategory' ";
                                mysqli_query($connect, $update_sql);
                                echo "Record with ID '$id' updated successfully.<br>";
                            }
                            else {
                                // The value does not exist
                                echo "No such value to update: '$name','$id', '$category', '$subcategory'.<br>";
                            }
                        } 
                    }
                }
