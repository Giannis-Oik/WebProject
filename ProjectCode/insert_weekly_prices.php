<?php // Arxeio pou eisageis tis evdomadiaies times ton proionton
session_start();
include 'db_conn.php';
...
if(isset($_SESSION['id']) && isset($_SESSION['user_name']))
{
            ...
            if(move_uploaded_file($file_tmp, $upload_directory . $file_name))
            {
                ...
                if ($array === null && json_last_error() !== JSON_ERROR_NONE) {
                    ...
                    foreach($array as $row){
                        ...
                        //Check an iparxei afti i timi
                        $check_sql = "SELECT * FROM weekly_prices WHERE  product_id = '$product_id' AND date = '$date' AND price = $price ";
                        $result = mysqli_query($conn, $check_sql);

                        if (mysqli_num_rows($result) > 0) {
                            echo "Value  '$product_id' , '$date' , '$price' already exists in the database.<br>";
                        } else {
                            //SQL insert
                            $sql = "INSERT INTO weekly_prices(product_id,date,price) VALUES ('$product_id','$date', '$price')";
    ...
    <html>
    <head>
        <title>Insert Weekly Prices</title>
        <link rel="stylesheet" type="text/css" href="style.css">
    </head>
    <body>
        <div>
            <h1>Insert Weekly Prices</h1>
