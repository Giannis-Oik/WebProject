            ...
            if(move_uploaded_file($file_tmp, $upload_directory . $file_name))
            {
                ...
                if ($array === null && json_last_error() !== JSON_ERROR_NONE) {
                    ...
                    foreach($array as $row){
                        ...
                        $check_sql = "SELECT * FROM prices WHERE  id = '$id' AND name = '$name' AND  category = '$category' AND date = '$date' AND price = $price ";
                        $result = mysqli_query($conn, $check_sql);

                        if (mysqli_num_rows($result) > 0) {
                            echo "Value '$name', '$id', '$category' , '$date' , '$price' already exists in the database.<br>";
                        } else {
                            // Perform SQL insertion
                            $sql = "INSERT INTO prices(id,name,category,date,price) VALUES ('$id','$name','$category','$date', '$price')";
